...
or if you feel that Nengo is running very slowly,
...
Nengo's only hard requirement is NumPy,
and it should be installed first.
...
- Mac OS X: Use Homebrew_ or Anaconda_
...
and all other Nengo dependencies.
...
every operating system has a package manager
that can install Python and NumPy.

- **Windows:** While Windows has the Chocolatey_ package manager,
  it currently does not have good support for NumPy.
  Anaconda_ is a capable package manager.
- **Mac OS X:** Homebrew_ has excellent Python support.
  After installing Homebrew, ``brew install python`` and ``pip install numpy``
  will give you a fast NumPy install.
- **Linux:** Linux distributions come with a package manager
  capable of installing Python and NumPy.
  Debian, Ubuntu, and others use ``apt``:
  ``sudo apt-get install python-numpy``.
  Fedora and others use ``yum``:
