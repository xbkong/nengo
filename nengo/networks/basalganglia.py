...
class BasalGanglia(nengo.Network):
    ...
    def __init__(self, dimensions, n_neurons_per_ensemble=100, radius=1.5,
        ...
        nengo.Connection(self.input, strD1.input, filter=None,
                         transform=self.ws * (1 + self.lg))
        nengo.Connection(self.input, strD2.input, filter=None,
                         transform=self.ws * (1 - self.le))
        nengo.Connection(self.input, stn.input, filter=None,
                         transform=self.wt)
